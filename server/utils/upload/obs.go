...
func NewHuaWeiObsClient() (client *obs.ObsClient, err error) {
	return obs.New(global.LRAG_CONFIG.HuaWeiObs.AccessKey, global.LRAG_CONFIG.HuaWeiObs.SecretKey, global.LRAG_CONFIG.HuaWeiObs.Endpoint)
}
...
func (o *Obs) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	input := &obs.PutObjectInput{
		PutObjectBasicInput: obs.PutObjectBasicInput{
			ObjectOperationInput: obs.ObjectOperationInput{
				Bucket: global.LRAG_CONFIG.HuaWeiObs.Bucket,
				Key:    filename,
			},
	...
	filepath := global.LRAG_CONFIG.HuaWeiObs.Path + "/" + filename
