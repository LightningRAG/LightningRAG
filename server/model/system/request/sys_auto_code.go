...
type AutoCode struct {
	...
	LragModel           bool                   `json:"lragModel" example:"false"`           // 是否使用 LightningRAG 默认 Model
	...
	return []model.SysApi{
		{
			...
			Description: "Create " + r.StructName,
			ApiGroup:    r.StructName,
			Method:      "POST",
		},
		{
			Path:        "/" + r.Abbreviation + "/" + "delete" + r.StructName,
			Description: "Delete " + r.StructName,
			ApiGroup:    r.StructName,
			Method:      "DELETE",
		},
		{
			Path:        "/" + r.Abbreviation + "/" + "delete" + r.StructName + "ByIds",
			Description: "Batch delete " + r.StructName,
			ApiGroup:    r.StructName,
			Method:      "DELETE",
		},
		{
			Path:        "/" + r.Abbreviation + "/" + "update" + r.StructName,
			Description: "Update " + r.StructName,
			ApiGroup:    r.StructName,
			Method:      "PUT",
		},
		{
			Path:        "/" + r.Abbreviation + "/" + "find" + r.StructName,
			Description: "Get " + r.StructName + " by ID",
			ApiGroup:    r.StructName,
			Method:      "GET",
		},
		{
			Path:        "/" + r.Abbreviation + "/" + "get" + r.StructName + "List",
			Description: "List " + r.StructName,
			ApiGroup:    r.StructName,
			Method:      "GET",
		},
	}
...
func (r *AutoCode) Pretreatment() error {
	r.Module = global.LRAG_CONFIG.AutoCode.Module
	...
	for i := 0; i < length; i++ {
		...
		if !r.LragModel && r.PrimaryField == nil && r.Fields[i].PrimaryKey {
