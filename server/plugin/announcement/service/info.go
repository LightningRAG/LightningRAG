...
import (
	...
	"github.com/LightningRAG/LightningRAG/server/plugin/announcement/model/request"
)
...
func (s *info) CreateInfo(info *model.Info) (err error) {
	err = global.LRAG_DB.Create(info).Error
	return err
}
...
func (s *info) DeleteInfo(ID string) (err error) {
	err = global.LRAG_DB.Delete(&model.Info{}, "id = ?", ID).Error
	return err
}
...
func (s *info) DeleteInfoByIds(IDs []string) (err error) {
	err = global.LRAG_DB.Delete(&[]model.Info{}, "id in ?", IDs).Error
	return err
}
...
func (s *info) UpdateInfo(info model.Info) (err error) {
	err = global.LRAG_DB.Model(&model.Info{}).Where("id = ?", info.ID).Updates(&info).Error
	return err
}
...
func (s *info) GetInfo(ID string) (info model.Info, err error) {
	err = global.LRAG_DB.Where("id = ?", ID).First(&info).Error
	return
}
...
func (s *info) GetInfoInfoList(info request.InfoSearch) (list []model.Info, total int64, err error) {
	...
	// 创建db
	db := global.LRAG_DB.Model(&model.Info{})
...
func (s *info) GetInfoDataSource() (res map[string][]map[string]any, err error) {
	...
	global.LRAG_DB.Table("sys_users").Select("nick_name as label,id as value").Scan(&userID)
	res["userID"] = userID
	return
}
