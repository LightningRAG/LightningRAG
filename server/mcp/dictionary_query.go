...
func (d *DictionaryQuery) Handle(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if dictType != "" {
		...
		if err != nil {
			global.LRAG_LOG.Error("查询字典失败", zap.Error(err))
		...
		db := global.LRAG_DB.Model(&system.SysDictionary{})
		...
		if err != nil {
			global.LRAG_LOG.Error("查询字典列表失败", zap.Error(err))
			return &mcp.CallToolResult{
				Content: []mcp.Content{
	...
	if err != nil {
		global.LRAG_LOG.Error("序列化响应失败", zap.Error(err))
		return &mcp.CallToolResult{
			Content: []mcp.Content{
