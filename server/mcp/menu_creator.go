...
**重要限制：**
- 当使用lrag_auto_generate工具且needCreatedModules=true时，模块创建会自动生成菜单项，不应调用此工具
...
func (m *MenuCreator) Handle(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	err = global.LRAG_DB.Where("name = ? AND path = ?", name, path).First(&createdMenu).Error
	if err != nil {
		global.LRAG_LOG.Warn("获取创建的菜单ID失败", zap.Error(err))
	}
