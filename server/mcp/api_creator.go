...
**重要限制：**
- 当使用lrag_auto_generate工具且needCreatedModules=true时，模块创建会自动生成API权限，不应调用此工具
...
func (a *ApiCreator) Handle(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	for _, apiReq := range apis {
		...
		if err != nil {
			...
			err = global.LRAG_DB.Where("path = ? AND method = ?", apiReq.Path, apiReq.Method).First(&createdApi).Error
			if err != nil {
				global.LRAG_LOG.Warn("获取创建的API ID失败", zap.Error(err))
			}
