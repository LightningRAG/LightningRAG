...
func (a *ApiLister) Handle(_ context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		global.LRAG_LOG.Error("获取数据库API失败", zap.Error(err))
	...
	if err != nil {
		global.LRAG_LOG.Error("获取gin路由API失败", zap.Error(err))
		errorResponse := ApiListResponse{
			Success: false,
	...
	global.LRAG_LOG.Info("API列表获取成功",
...
func (a *ApiLister) getDatabaseApis() ([]ApiInfo, error) {
	...
	err := global.LRAG_DB.Model(&system.SysApi{}).Order("api_group ASC, path ASC").Find(&apis).Error
	if err != nil {
		return nil, err
	}
...
func (a *ApiLister) getGinApis() ([]ApiInfo, error) {
	// 从gin路由信息中获取所有API
	var result []ApiInfo
	for _, route := range global.LRAG_ROUTERS {
		result = append(result, ApiInfo{
			Path:   route.Path,
			Method: route.Method,
			Source: "gin",
