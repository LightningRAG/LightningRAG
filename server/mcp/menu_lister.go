...
func (m *MenuLister) Handle(_ context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		global.LRAG_LOG.Error("获取菜单列表失败", zap.Error(err))
	...
	if err != nil {
		global.LRAG_LOG.Error("序列化菜单响应失败", zap.Error(err))
		return &mcp.CallToolResult{
			Content: []mcp.Content{
				mcp.TextContent{
					Type: "text",
...
func (m *MenuLister) getAllMenus() ([]system.SysBaseMenu, error) {
	...
	err := global.LRAG_DB.Order("sort").Preload("Parameters").Preload("MenuBtn").Find(&menus).Error
	if err != nil {
		return nil, err
	}
	return menus, nil
