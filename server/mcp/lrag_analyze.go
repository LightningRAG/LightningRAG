...
// 注册工具
func init() {
	RegisterTool(&LRAGAnalyzer{})
}

// LRAGAnalyzer LRAG分析器 - 用于分析当前功能是否需要创建独立的package和module
type LRAGAnalyzer struct{}
...
// New 创建LRAG分析器工具
func (g *LRAGAnalyzer) New() mcp.Tool {
	return mcp.NewTool("lrag_analyze",
...
// Handle 处理分析请求
func (g *LRAGAnalyzer) Handle(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// 解析请求参数
...
func (g *LRAGAnalyzer) performAnalysis(ctx context.Context, req AnalyzeRequest) (*AnalyzeResponse, error) {
	// 1. 获取数据库中的包信息
	var packages []model.SysAutoCodePackage
	if err := global.LRAG_DB.Find(&packages).Error; err != nil {
		return nil, fmt.Errorf("获取包信息失败: %v", err)
	}

	// 2. 获取历史记录
	var histories []model.SysAutoCodeHistory
	if err := global.LRAG_DB.Find(&histories).Error; err != nil {
	...
	for _, pkg := range packages {
		...
		if err != nil {
			global.LRAG_LOG.Warn(fmt.Sprintf("检查包 %s 是否为空时出错: %v", pkg.PackageName, err))
			continue
		}
		...
		if isEmpty {
			...
			if err := g.removeEmptyPackageFolder(pkg.PackageName, pkg.Template); err != nil {
				global.LRAG_LOG.Warn(fmt.Sprintf("删除空包文件夹 %s 失败: %v", pkg.PackageName, err))
			} else {
				cleanupInfo.DeletedPackages = append(cleanupInfo.DeletedPackages, pkg.PackageName)
			}

			// 删除数据库记录
			if err := global.LRAG_DB.Delete(&pkg).Error; err != nil {
				global.LRAG_LOG.Warn(fmt.Sprintf("删除包数据库记录 %s 失败: %v", pkg.PackageName, err))
			}
	...
	if len(dirtyHistoryIDs) > 0 {
		if err := global.LRAG_DB.Delete(&model.SysAutoCodeHistory{}, "id IN ?", dirtyHistoryIDs).Error; err != nil {
			global.LRAG_LOG.Warn(fmt.Sprintf("删除脏历史记录失败: %v", err))
		} else {
			global.LRAG_LOG.Info(fmt.Sprintf("成功删除 %d 条脏历史记录", len(dirtyHistoryIDs)))
		}

		// 清理相关的API和菜单记录
		if err := g.cleanupRelatedApiAndMenus(dirtyHistoryIDs); err != nil {
			global.LRAG_LOG.Warn(fmt.Sprintf("清理相关API和菜单记录失败: %v", err))
		}
	}
	...
	if err != nil {
		global.LRAG_LOG.Warn(fmt.Sprintf("扫描预设计模块失败: %v", err))
	...
	err = global.LRAG_DB.Table("sys_dictionaries").Find(&dictionaries, "deleted_at is null").Error
	if err != nil {
		global.LRAG_LOG.Warn(fmt.Sprintf("获取字典信息失败: %v", err))
...
func (g *LRAGAnalyzer) isPackageFolderEmpty(packageName, template string) (bool, error) {
	// 根据模板类型确定基础路径
	var basePath string
	if template == "plugin" {
		basePath = filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "plugin", packageName)
	} else {
		basePath = filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "api", "v1", packageName)
	}
...
func (g *LRAGAnalyzer) hasGoFilesRecursive(dirPath string) (bool, error) {
...
func (g *LRAGAnalyzer) removeEmptyPackageFolder(packageName, template string) error {
	var basePath string
	if template == "plugin" {
		basePath = filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "plugin", packageName)
	} else {
		...
		paths := []string{
			filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "api", "v1", packageName),
			filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "model", packageName),
			filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "router", packageName),
			filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "service", packageName),
		}
...
func (g *LRAGAnalyzer) removeDirectoryIfExists(dirPath string) error {
...
func (g *LRAGAnalyzer) cleanupRelatedApiAndMenus(historyIDs []uint) error {
	if len(historyIDs) == 0 {
		return nil
	}
	...
	global.LRAG_LOG.Info(fmt.Sprintf("清理历史记录ID %v 相关的API和菜单记录", historyIDs))
...
func (g *LRAGAnalyzer) scanPredesignedModules() ([]PredesignedModuleInfo, error) {
	// 获取autocode配置路径
	autocodeRoot := global.LRAG_CONFIG.AutoCode.Root
	if autocodeRoot == "" {
	...
	// 扫描plugin目录
	pluginModules, err := g.scanPluginModules(filepath.Join(autocodeRoot, global.LRAG_CONFIG.AutoCode.Server, "plugin"))
	if err != nil {
		global.LRAG_LOG.Warn(fmt.Sprintf("扫描plugin模块失败: %v", err))
	} else {
		modules = append(modules, pluginModules...)
	}

	// 扫描model目录
	modelModules, err := g.scanModelModules(filepath.Join(autocodeRoot, global.LRAG_CONFIG.AutoCode.Server, "model"))
	if err != nil {
		global.LRAG_LOG.Warn(fmt.Sprintf("扫描model模块失败: %v", err))
	} else {
...
func (g *LRAGAnalyzer) scanPluginModules(pluginDir string) ([]PredesignedModuleInfo, error) {
	var modules []PredesignedModuleInfo
	...
	for _, entry := range entries {
		if entry.IsDir() {
			...
			if _, err := os.Stat(modelDir); err == nil {
				...
				if err != nil {
					global.LRAG_LOG.Warn(fmt.Sprintf("扫描插件 %s 的模块失败: %v", pluginName, err))
					continue
				}
				modules = append(modules, pluginModules...)
			}
...
func (g *LRAGAnalyzer) scanModelModules(modelDir string) ([]PredesignedModuleInfo, error) {
	var modules []PredesignedModuleInfo
	...
	for _, entry := range entries {
		if entry.IsDir() {
			...
			if err != nil {
				global.LRAG_LOG.Warn(fmt.Sprintf("扫描包 %s 的模块失败: %v", packageName, err))
				continue
			}
...
func (g *LRAGAnalyzer) scanModulesInDirectory(dir, packageName, template string) ([]PredesignedModuleInfo, error) {
	var modules []PredesignedModuleInfo
