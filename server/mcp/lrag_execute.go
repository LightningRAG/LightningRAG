...
// 注册工具
func init() {
	RegisterTool(&LRAGExecutor{})
}

// LRAGExecutor LRAG代码生成器
type LRAGExecutor struct{}
...
// New 创建LRAG代码生成执行器工具
func (g *LRAGExecutor) New() mcp.Tool {
	return mcp.NewTool("lrag_execute",
		mcp.WithDescription(`**LRAG代码生成执行器：直接执行代码生成，无需确认步骤**

**核心功能：**
根据需求分析和当前的包信息判断是否调用，直接生成代码。支持批量创建多个模块、自动创建包、模块、字典等。

**使用场景：**
在lrag_analyze获取了当前的包信息和字典信息之后，如果已经包含了可以使用的包和模块，那就不要调用本mcp。根据分析结果直接生成代码，适用于自动化代码生成流程。
				...
				"modulesInfo": map[string]interface{}{
					...
					"items": map[string]interface{}{
						...
						"properties": map[string]interface{}{
							...
							"lragModel":           map[string]interface{}{"type": "boolean", "description": "是否使用 LightningRAG 默认模型（固定为true），自动包含ID、CreatedAt、UpdatedAt、DeletedAt字段"},
							...
							"fields": map[string]interface{}{
								...
								"items": map[string]interface{}{
									...
									"properties": map[string]interface{}{
										...
										"primaryKey":      map[string]interface{}{"type": "boolean", "description": "是否主键（lragModel=false时必须有一个字段为true）"},
										"dataSource": map[string]interface{}{
											"type":        "object",
											"description": "数据源配置，用于配置字段的关联表信息。获取表名提示：可在 server/model 和 plugin/xxx/model 目录下查看对应模块的 TableName() 接口实现获取实际表名（如 SysUser 的表名为 sys_users）。获取数据库名提示：主数据库通常使用 lrag（默认数据库标识），多数据库可在 config.yaml 的 db-list 配置中查看可用数据库的 alias-name 字段，如果用户未提及关联多数据库信息则使用默认数据库，默认数据库的情况下 dbName填写为空",
											"properties": map[string]interface{}{
...
func (g *LRAGExecutor) Handle(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if result.Success && originalRequirement != "" {
		global.LRAG_LOG.Info("执行完成，返回生成的文件路径供AI进行代码复检...")
...
func (g *LRAGExecutor) validateExecutionPlan(plan *ExecutionPlan) error {
	// 验证基本字段
	if plan.PackageName == "" {
	...
	if plan.NeedCreatedModules {
		...
		for moduleIndex, moduleInfo := range plan.ModulesInfo {
				...
				if field.DataSource != nil {
					...
					if associationValue == 2 {
						if field.FieldType != "array" {
							global.LRAG_LOG.Info(fmt.Sprintf("模块 %d 字段 %d：检测到一对多关联(association=2)，自动将 fieldType 从 '%s' 修改为 'array'", moduleIndex+1, i+1, field.FieldType))
			...
			// 验证主键设置
			if !moduleInfo.LragModel {
				// 当不使用LRAG模型时，必须有且仅有一个字段设置为主键
				primaryKeyCount := 0
				...
				if primaryKeyCount == 0 {
					return fmt.Errorf("模块 %d：当 lragModel=false 时，必须有一个字段的 primaryKey=true", moduleIndex+1)
				}
				if primaryKeyCount > 1 {
					return fmt.Errorf("模块 %d：当 lragModel=false 时，只能有一个字段的 primaryKey=true", moduleIndex+1)
				}
			} else {
				// 当使用LRAG模型时，所有字段的primaryKey都应该为false
				for i, field := range moduleInfo.Fields {
					if field.PrimaryKey {
						return fmt.Errorf("模块 %d：当 lragModel=true 时，字段 %d 的 primaryKey 应该为 false，系统会自动创建ID主键", moduleIndex+1, i+1)
					}
				}
			}
		}
...
func (g *LRAGExecutor) executeCreation(ctx context.Context, plan *ExecutionPlan) *ExecuteResponse {
...
func (g *LRAGExecutor) buildDirectoryStructure(plan *ExecutionPlan) map[string]string {
	paths := make(map[string]string)

	// 获取配置信息
	autoCodeConfig := global.LRAG_CONFIG.AutoCode
...
func (g *LRAGExecutor) collectExpectedFilePaths(plan *ExecutionPlan) []string {
	var paths []string
...
func (g *LRAGExecutor) checkDictionaryExists(dictType string) (bool, error) {
...
func (g *LRAGExecutor) createDictionariesFromInfo(ctx context.Context, dictionariesInfo []*DictionaryGenerateRequest) string {
	var messages []string
	dictionaryService := service.ServiceGroupApp.SystemServiceGroup.DictionaryService
	...
	for _, dictInfo := range dictionariesInfo {
		...
		if !exists {
			...
			err = global.LRAG_DB.Where("type = ?", dictInfo.DictType).First(&createdDict).Error
			if err != nil {
			...
			if len(dictInfo.Options) > 0 {
				...
				for _, option := range dictInfo.Options {
					...
					if err != nil {
						global.LRAG_LOG.Warn("创建字典详情项失败", zap.Error(err))
					} else {
						successCount++
					}
				}
