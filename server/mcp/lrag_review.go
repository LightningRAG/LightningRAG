...
// LRAGReviewer LightningRAG 代码审查工具
type LRAGReviewer struct{}

// init 注册工具
func init() {
	RegisterTool(&LRAGReviewer{})
}

// ReviewRequest 审查请求结构
type ReviewRequest struct {
	UserRequirement string   `json:"userRequirement"` // 经过requirement_analyze后的用户需求
	GeneratedFiles  []string `json:"generatedFiles"`  // lrag_execute创建的文件列表
}
...
// New 创建 LightningRAG 代码审查工具
func (g *LRAGReviewer) New() mcp.Tool {
	return mcp.NewTool("lrag_review",
		mcp.WithDescription(`**LightningRAG 代码审查工具 - 在 lrag_execute 调用后使用**

**核心功能：**
- 接收经过 requirement_analyze 处理的用户需求和 lrag_execute 生成的文件列表
- 分析生成的代码是否满足用户的原始需求
- 检查是否涉及到关联、交互等复杂功能
- 如果代码不满足需求，提供调整建议和新的prompt

**使用场景：**
- 在lrag_execute成功执行后调用
- 用于验证生成的代码是否完整满足用户需求
- 检查模块间的关联关系是否正确实现
- 发现缺失的交互功能或业务逻辑
...
- 提供的调整建议应该具体可执行`),
		...
		mcp.WithString("generatedFiles",
			mcp.Description("lrag_execute 创建的文件列表，JSON字符串格式，包含所有生成的后端和前端文件路径"),
			mcp.Required(),
		),
	)
}
...
// Handle 处理审查请求
func (g *LRAGReviewer) Handle(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// 获取用户需求
...
func (g *LRAGReviewer) generateAdjustmentPrompt(userRequirement string, generatedFiles []string) string {
	var prompt strings.Builder

	prompt.WriteString("🔧 **代码调整指导 Prompt：**\n\n")
	prompt.WriteString(fmt.Sprintf("**用户的原始需求为：** %s\n\n", userRequirement))
	prompt.WriteString("**经过 LightningRAG 生成后的文件有如下内容：**\n")
	...
	prompt.WriteString("5. 遵循 LightningRAG 框架的开发规范和最佳实践\n")
