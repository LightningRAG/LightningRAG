...
func (m *mongo) Initialization() error {
	...
	if global.LRAG_CONFIG.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	config := &qmgo.Config{
		Uri:              global.LRAG_CONFIG.Mongo.Uri(),
		Coll:             global.LRAG_CONFIG.Mongo.Coll,
		Database:         global.LRAG_CONFIG.Mongo.Database,
		MinPoolSize:      &global.LRAG_CONFIG.Mongo.MinPoolSize,
		MaxPoolSize:      &global.LRAG_CONFIG.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &global.LRAG_CONFIG.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &global.LRAG_CONFIG.Mongo.ConnectTimeoutMs,
	}
	if global.LRAG_CONFIG.Mongo.Username != "" && global.LRAG_CONFIG.Mongo.Password != "" {
		config.Auth = &qmgo.Credential{
			Username:   global.LRAG_CONFIG.Mongo.Username,
			Password:   global.LRAG_CONFIG.Mongo.Password,
			AuthSource: global.LRAG_CONFIG.Mongo.AuthSource,
		}
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := global.LRAG_MONGO.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = global.LRAG_MONGO.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
