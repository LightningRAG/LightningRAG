...
func (sysVersionApi *SysVersionApi) GetSysVersionPublic(c *gin.Context) {
	...
	response.OkWithDetailed(gin.H{
		"info": i18n.Msg(c, "sys.version.public_unauth_notice"),
	}, i18n.Msg(c, "common.fetch_success"), c)
}
...
func (sysVersionApi *SysVersionApi) ExportVersion(c *gin.Context) {
	...
	if len(req.MenuIds) > 0 {
		...
		if err != nil {
			global.LRAG_LOG.Error("获取菜单数据失败!", zap.Error(err))
			response.FailWithMessage(i18n.Msgf(c, "sys.version.menu_fetch_failed", err.Error()), c)
			return
		}
	}
	...
	if len(req.ApiIds) > 0 {
		...
		if err != nil {
			global.LRAG_LOG.Error("获取API数据失败!", zap.Error(err))
			response.FailWithMessage(i18n.Msgf(c, "sys.version.api_fetch_failed", err.Error()), c)
			return
		}
	}
	...
	if len(req.DictIds) > 0 {
		...
		if err != nil {
			global.LRAG_LOG.Error("获取字典数据失败!", zap.Error(err))
			response.FailWithMessage(i18n.Msgf(c, "sys.version.dict_fetch_failed", err.Error()), c)
			return
		}
	}
	...
	if err != nil {
		global.LRAG_LOG.Error("JSON序列化失败!", zap.Error(err))
		response.FailWithMessage(i18n.Msgf(c, "common.json_marshal_failed_detail", err.Error()), c)
		return
	}
	...
	if err != nil {
		global.LRAG_LOG.Error("保存版本记录失败!", zap.Error(err))
		response.FailWithMessage(i18n.Msgf(c, "sys.version.save_record_failed", err.Error()), c)
		return
	}

	response.OkWithMessage(i18n.Msg(c, "sys.version.create_release_success"), c)
}
...
func (sysVersionApi *SysVersionApi) DownloadVersionJson(c *gin.Context) {
	...
	ID := c.Query("ID")
	if ID == "" {
		response.FailWithMessage(i18n.Msg(c, "validation.version_id_required"), c)
		return
	}
	...
	if err != nil {
		global.LRAG_LOG.Error("获取版本记录失败!", zap.Error(err))
		response.FailWithMessage(i18n.Msgf(c, "sys.version.get_record_failed", err.Error()), c)
		return
	}
...
func (sysVersionApi *SysVersionApi) ImportVersion(c *gin.Context) {
	...
	if err != nil {
		response.FailWithMessage(i18n.Msgf(c, "common.json_parse_failed_detail", err.Error()), c)
		return
	}

	// 验证数据格式
	if importData.VersionInfo.Name == "" || importData.VersionInfo.Code == "" {
		response.FailWithMessage(i18n.Msg(c, "sys.version.payload_invalid"), c)
		return
	}
	...
	if len(importData.ExportMenu) > 0 {
		if err := sysVersionService.ImportMenus(ctx, importData.ExportMenu); err != nil {
			global.LRAG_LOG.Error("导入菜单失败!", zap.Error(err))
			response.FailWithMessage(i18n.Msgf(c, "sys.version.import_menu_failed", err.Error()), c)
			return
		}
	}
	...
	if len(importData.ExportApi) > 0 {
		if err := sysVersionService.ImportApis(importData.ExportApi); err != nil {
			global.LRAG_LOG.Error("导入API失败!", zap.Error(err))
			response.FailWithMessage(i18n.Msgf(c, "sys.version.import_api_failed", err.Error()), c)
			return
		}
	}
	...
	if len(importData.ExportDictionary) > 0 {
		if err := sysVersionService.ImportDictionaries(importData.ExportDictionary); err != nil {
			global.LRAG_LOG.Error("导入字典失败!", zap.Error(err))
			response.FailWithMessage(i18n.Msgf(c, "sys.version.import_dict_failed", err.Error()), c)
			return
		}
	}
	...
	err = sysVersionService.CreateSysVersion(ctx, &version)
	if err != nil {
		global.LRAG_LOG.Error("保存导入记录失败!", zap.Error(err))
		// 这里不返回错误，因为数据已经导入成功
	}
