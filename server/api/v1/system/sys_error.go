...
func (sysErrorApi *SysErrorApi) GetSysErrorSolution(c *gin.Context) {
	...
	ID := c.Query("id")
	if ID == "" {
		response.FailWithMessage(i18n.Msg(c, "validation.missing_id"), c)
		return
	}

	err := sysErrorService.GetSysErrorSolution(ctx, ID)
	if err != nil {
		global.LRAG_LOG.Error("处理触发失败!", zap.Error(err))
		response.FailWithMessage(i18n.Msgf(c, "sys.error.trigger_ai_failed", err.Error()), c)
		return
	}

	response.OkWithMessage(i18n.Msg(c, "sys.error.submitted_to_ai"), c)
}
