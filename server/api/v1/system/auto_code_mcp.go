...
func (a *AutoCodeTemplateApi) MCP(c *gin.Context) {
	...
	if err != nil {
		...
		global.LRAG_LOG.Error(err.Error())
		return
	}
	response.OkWithMessage(i18n.Msgf(c, "sys.mcp.create_success_path", toolFilePath), c)
}
...
func (a *AutoCodeTemplateApi) MCPList(c *gin.Context) {
	...
	baseUrl := fmt.Sprintf("http://127.0.0.1:%d%s", global.LRAG_CONFIG.System.Addr, global.LRAG_CONFIG.MCP.SSEPath)

	testClient, err := client.NewClient(baseUrl, "testClient", "v1.0.0", global.LRAG_CONFIG.MCP.Name)
	...
	mcpServerConfig := map[string]interface{}{
		"mcpServers": map[string]interface{}{
			global.LRAG_CONFIG.MCP.Name: map[string]string{
				"url": baseUrl,
			},
		},
	}
...
func (a *AutoCodeTemplateApi) MCPTest(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&testRequest); err != nil {
		response.FailWithMessage(i18n.Msgf(c, "sys.mcp.parse_params_failed", err.Error()), c)
		return
	}
	...
	testClient, err := client.NewClient(baseUrl, "testClient", "v1.0.0", global.LRAG_CONFIG.MCP.Name)
	if err != nil {
		response.FailWithMessage(i18n.Msgf(c, "sys.mcp.create_client_failed", err.Error()), c)
		return
	}
	defer testClient.Close()
	...
	if err != nil {
		response.FailWithMessage(i18n.Msgf(c, "sys.mcp.init_connection_failed", err.Error()), c)
		return
	}
	...
	if err != nil {
		response.FailWithMessage(i18n.Msgf(c, "sys.mcp.call_failed", err.Error()), c)
		return
	}

	// 处理响应结果
	if len(result.Content) == 0 {
		response.FailWithMessage(i18n.Msg(c, "sys.mcp.tool_returned_empty"), c)
		return
	}
