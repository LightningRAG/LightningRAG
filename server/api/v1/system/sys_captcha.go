...
func (b *BaseApi) Captcha(c *gin.Context) {
	...
	// 字符,公式,验证码配置
	// 生成默认数字的driver
	driver := base64Captcha.NewDriverDigit(global.LRAG_CONFIG.Captcha.ImgHeight, global.LRAG_CONFIG.Captcha.ImgWidth, global.LRAG_CONFIG.Captcha.KeyLong, 0.7, 80)
	// cp := base64Captcha.NewCaptcha(driver, store.UseWithCtx(c))   // v8下使用redis
	cp := base64Captcha.NewCaptcha(driver, store)
	id, b64s, _, err := cp.Generate()
	if err != nil {
		global.LRAG_LOG.Error("验证码获取失败!", zap.Error(err))
		response.FailWithMessage(i18n.Msg(c, "sys.captcha.fetch_failed"), c)
		return
	}
	response.OkWithDetailed(systemRes.SysCaptchaResponse{
		CaptchaId:     id,
		PicPath:       b64s,
		CaptchaLength: global.LRAG_CONFIG.Captcha.KeyLong,
		OpenCaptcha:   oc,
	}, i18n.Msg(c, "sys.captcha.fetch_success"), c)
}
