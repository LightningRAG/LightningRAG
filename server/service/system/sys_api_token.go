...
func (apiVersion *ApiTokenService) CreateApiToken(apiToken system.SysApiToken, days int) (string, error) {
	...
	if err := global.LRAG_DB.Where("id = ?", apiToken.UserID).First(&user).Error; err != nil {
	...
	j := &utils.JWT{SigningKey: []byte(global.LRAG_CONFIG.JWT.SigningKey)} // 唯一不同的部分是过期时间
	...
	bf, _ := utils.ParseDuration(global.LRAG_CONFIG.JWT.BufferTime)
	...
	claims := sysReq.CustomClaims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			Audience:  jwt.ClaimStrings{"LRAG"},
			NotBefore: jwt.NewNumericDate(time.Now().Add(-1000)),
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(expireTime)),
			Issuer:    global.LRAG_CONFIG.JWT.Issuer,
		},
	}
	...
	err = global.LRAG_DB.Create(&apiToken).Error
	return token, err
}
...
func (apiVersion *ApiTokenService) GetApiTokenList(info sysReq.SysApiTokenSearch) (list []system.SysApiToken, total int64, err error) {
	...
	db := global.LRAG_DB.Model(&system.SysApiToken{})
...
func (apiVersion *ApiTokenService) DeleteApiToken(id uint) error {
	...
	err := global.LRAG_DB.First(&apiToken, id).Error
	if err != nil {
		return err
	}
	...
	return global.LRAG_DB.Model(&apiToken).Update("status", false).Error
}
