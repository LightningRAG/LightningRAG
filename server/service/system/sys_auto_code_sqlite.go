...
func (a *autoCodeSqlite) GetDB(businessDB string) (data []response.Db, err error) {
	...
	if businessDB == "" {
		err = global.LRAG_DB.Raw(sql).Find(&databaseList).Error
	} else {
		err = global.LRAG_DBList[businessDB].Raw(sql).Find(&databaseList).Error
	}
	...
	// entities = append(entities, response.Db{global.LRAG_CONFIG.Sqlite.Dbname})
	return entities, err
}
...
func (a *autoCodeSqlite) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	tabelNames := []string{}
	if businessDB == "" {
		err = global.LRAG_DB.Raw(sql).Find(&tabelNames).Error
	} else {
		err = global.LRAG_DBList[businessDB].Raw(sql).Find(&tabelNames).Error
	}
...
func (a *autoCodeSqlite) GetColumn(businessDB string, tableName string, dbName string) (data []response.Column, err error) {
	if err := validateAutoCodeIdent(tableName, "表名"); err != nil {
		return nil, err
	}
	...
	if businessDB == "" {
		err = global.LRAG_DB.Raw(sql).Scan(&columnInfos).Error
	} else {
		err = global.LRAG_DBList[businessDB].Raw(sql).Scan(&columnInfos).Error
	}
