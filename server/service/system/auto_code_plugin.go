...
import (
	...
	pluginUtils "github.com/LightningRAG/LightningRAG/server/plugin/plugin-tool/utils"
	"github.com/LightningRAG/LightningRAG/server/utils"
	ast "github.com/LightningRAG/LightningRAG/server/utils/ast"
...
func (s *autoCodePlugin) Install(file *multipart.FileHeader) (web, server int, err error) {
	const LRAGPLUGINPATH = "./lrag-plug-temp/"
	defer os.RemoveAll(LRAGPLUGINPATH)
	_, err = os.Stat(LRAGPLUGINPATH)
	if os.IsNotExist(err) {
		os.Mkdir(LRAGPLUGINPATH, os.ModePerm)
	}
	...
	out, err := os.Create(LRAGPLUGINPATH + file.Filename)
	if err != nil {
		return -1, -1, err
	}
	...
	paths, err := utils.Unzip(LRAGPLUGINPATH+file.Filename, LRAGPLUGINPATH)
	...
	if len(serverPlugin) != 0 {
		...
		err = installation(serverPlugin, global.LRAG_CONFIG.AutoCode.Server, global.LRAG_CONFIG.AutoCode.Server)
		if err != nil {
	...
	if len(webPlugin) != 0 {
		err = installation(webPlugin, global.LRAG_CONFIG.AutoCode.Server, global.LRAG_CONFIG.AutoCode.Web)
		if err != nil {
			return webIndex, serverIndex, err
		}
	}
...
func installation(path string, formPath string, toPath string) error {
	...
	var form = filepath.Join(global.LRAG_CONFIG.AutoCode.Root, formPath, path)
	var to = filepath.Join(global.LRAG_CONFIG.AutoCode.Root, toPath, "plugin")
...
func ensurePluginRegisterImport(packageName string) error {
	module := strings.TrimSpace(global.LRAG_CONFIG.AutoCode.Module)
	if module == "" {
	...
	registerPath := filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "plugin", "register.go")
...
func (s *autoCodePlugin) PubPlug(plugName string) (zipPath string, err error) {
	...
	webPath := filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Web, "plugin", plugName)
	serverPath := filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "plugin", plugName)
	// 创建一个新的zip文件
	...
	return filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, fileName), nil
}

func (s *autoCodePlugin) InitMenu(menuInfo request.InitMenu) (err error) {
	menuPath := filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "plugin", menuInfo.PlugName, "initialize", "menu.go")
	...
	// 查询菜单及其关联的参数和按钮
	err = global.LRAG_DB.Preload("Parameters").Preload("MenuBtn").Find(&menus, "id in (?)", menuInfo.Menus).Error
	if err != nil {
		return err
	}
...
func (s *autoCodePlugin) InitAPI(apiInfo request.InitApi) (err error) {
	apiPath := filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "plugin", apiInfo.PlugName, "initialize", "api.go")
	...
	err = global.LRAG_DB.Find(&apis, "id in (?)", apiInfo.APIs).Error
	if err != nil {
		return err
	}
...
func (s *autoCodePlugin) InitDictionary(dictInfo request.InitDictionary) (err error) {
	dictPath := filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "plugin", dictInfo.PlugName, "initialize", "dictionary.go")
	...
	err = global.LRAG_DB.Preload("SysDictionaryDetails").Find(&dictionaries, "id in (?)", dictInfo.Dictionaries).Error
	if err != nil {
		return err
	}
...
func (s *autoCodePlugin) Remove(pluginName string, pluginType string) (err error) {
	...
	if pluginType == "web" || pluginType == "full" {
		webDir := filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Web, "plugin", pluginName)
	...
	if pluginType == "server" || pluginType == "full" {
		serverDir := filepath.Join(global.LRAG_CONFIG.AutoCode.Root, global.LRAG_CONFIG.AutoCode.Server, "plugin", pluginName)
	...
	if len(menus) > 0 {
		for _, menu := range menus {
			...
			if err := global.LRAG_DB.Where("name = ?", menu.Name).First(&dbMenu).Error; err == nil {
				// 获取该菜单及其所有子菜单的ID
				var menuIds []int
	...
	if len(apis) > 0 {
		for _, api := range apis {
			...
			if err := global.LRAG_DB.Where("path = ? AND method = ?", api.Path, api.Method).First(&dbApi).Error; err == nil {
	...
	if len(dicts) > 0 {
		for _, dict := range dicts {
			...
			if err := global.LRAG_DB.Where("type = ?", dict.Type).First(&dbDict).Error; err == nil {
...
func GetMenuIds(menu system.SysBaseMenu, ids *[]int) {
	*ids = append(*ids, int(menu.ID))
	...
	global.LRAG_DB.Where("parent_id = ?", menu.ID).Find(&children)
