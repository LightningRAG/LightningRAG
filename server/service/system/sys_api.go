...
func (apiService *ApiService) CreateApi(api system.SysApi) (err error) {
	if !errors.Is(global.LRAG_DB.Where("path = ? AND method = ?", api.Path, api.Method).First(&system.SysApi{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同api")
	}
	return global.LRAG_DB.Create(&api).Error
}

func (apiService *ApiService) GetApiGroups() (groups []string, groupApiMap map[string]string, err error) {
	var apis []system.SysApi
	err = global.LRAG_DB.Find(&apis).Error
	if err != nil {
		return
	}
...
func (apiService *ApiService) SyncApi() (newApis, deleteApis, ignoreApis []system.SysApi, err error) {
	...
	err = global.LRAG_DB.Find(&ignores).Error
	if err != nil {
		return
	}
	...
	for i := range global.LRAG_ROUTERS {
		ignoresFlag := false
		for j := range ignores {
			if ignores[j].Path == global.LRAG_ROUTERS[i].Path && ignores[j].Method == global.LRAG_ROUTERS[i].Method {
				ignoresFlag = true
			}
		}
		if !ignoresFlag {
			cacheApis = append(cacheApis, system.SysApi{
				Path:   global.LRAG_ROUTERS[i].Path,
				Method: global.LRAG_ROUTERS[i].Method,
			})
		}
	}
...
func (apiService *ApiService) IgnoreApi(ignoreApi system.SysIgnoreApi) (err error) {
	if ignoreApi.Flag {
		return global.LRAG_DB.Create(&ignoreApi).Error
	}
	return global.LRAG_DB.Unscoped().Delete(&ignoreApi, "path = ? AND method = ?", ignoreApi.Path, ignoreApi.Method).Error
}
...
func (apiService *ApiService) DeleteApi(api system.SysApi) (err error) {
	...
	err = global.LRAG_DB.First(&entity, "id = ?", api.ID).Error // 根据id查询api记录
	if errors.Is(err, gorm.ErrRecordNotFound) {                 // api记录不存在
		return err
	}
	err = global.LRAG_DB.Delete(&entity).Error
	if err != nil {
		return err
	}
...
func (apiService *ApiService) GetAPIInfoList(api system.SysApi, info request.PageInfo, order string, desc bool) (list interface{}, total int64, err error) {
	...
	db := global.LRAG_DB.Model(&system.SysApi{})
...
func (apiService *ApiService) GetAllApis(authorityID uint) (apis []system.SysApi, err error) {
	...
	err = global.LRAG_DB.Order("id desc").Find(&apis).Error
	if parentAuthorityID == 0 || !global.LRAG_CONFIG.System.UseStrictAuth {
		return
	}
...
func (apiService *ApiService) GetApiById(id int) (api system.SysApi, err error) {
	err = global.LRAG_DB.First(&api, "id = ?", id).Error
	return
}
...
func (apiService *ApiService) UpdateApi(api system.SysApi) (err error) {
	...
	err = global.LRAG_DB.First(&oldA, "id = ?", api.ID).Error
	if oldA.Path != api.Path || oldA.Method != api.Method {
		var duplicateApi system.SysApi
		if ferr := global.LRAG_DB.First(&duplicateApi, "path = ? AND method = ?", api.Path, api.Method).Error; ferr != nil {
	...
	return global.LRAG_DB.Save(&api).Error
}
