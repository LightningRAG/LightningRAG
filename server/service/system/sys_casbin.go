...
func (casbinService *CasbinService) UpdateCasbinApi(oldPath string, newPath string, oldMethod string, newMethod string) error {
	err := global.LRAG_DB.Model(&gormadapter.CasbinRule{}).Where("v1 = ? AND v2 = ?", oldPath, oldMethod).Updates(map[string]interface{}{
		"v1": newPath,
		"v2": newMethod,
	}).Error
	if err != nil {
...
func (casbinService *CasbinService) FreshCasbin() (err error) {
	e := utils.GetCasbin()
	if e == nil {
		return errors.New("casbin: enforcer not initialized (database not ready)")
	}
	return e.LoadPolicy()
}
...
func (casbinService *CasbinService) GetAuthoritiesByApi(path, method string) (authorityIds []uint, err error) {
	...
	err = global.LRAG_DB.Where("ptype = 'p' AND v1 = ? AND v2 = ?", path, method).Find(&rules).Error
	if err != nil {
		return nil, err
	}
	for _, r := range rules {
