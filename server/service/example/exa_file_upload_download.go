...
func (e *FileUploadAndDownloadService) Upload(file example.ExaFileUploadAndDownload) error {
	return global.LRAG_DB.Create(&file).Error
}
...
func (e *FileUploadAndDownloadService) FindFile(id uint) (example.ExaFileUploadAndDownload, error) {
	...
	err := global.LRAG_DB.Where("id = ?", id).First(&file).Error
	return file, err
}
...
func (e *FileUploadAndDownloadService) DeleteFile(file example.ExaFileUploadAndDownload) (err error) {
	...
	if strings.TrimSpace(fileFromDb.Key) != "" {
		if delErr := oss.DeleteFile(fileFromDb.Key); delErr != nil {
			if errors.Is(delErr, upload.ErrFileNotFound) {
				global.LRAG_LOG.Warn("物理文件已不存在，继续删除数据库记录",
					zap.Uint("id", file.ID), zap.String("key", fileFromDb.Key))
			} else {
				return fmt.Errorf("文件删除失败: %w", delErr)
			}
		}
	}
	err = global.LRAG_DB.Where("id = ?", file.ID).Unscoped().Delete(&file).Error
	return err
}
...
func (e *FileUploadAndDownloadService) EditFileName(file example.ExaFileUploadAndDownload) (err error) {
	var fileFromDb example.ExaFileUploadAndDownload
	return global.LRAG_DB.Where("id = ?", file.ID).First(&fileFromDb).Update("name", file.Name).Error
}
...
func (e *FileUploadAndDownloadService) GetFileRecordInfoList(info request.ExaAttachmentCategorySearch) (list []example.ExaFileUploadAndDownload, total int64, err error) {
	...
	db := global.LRAG_DB.Model(&example.ExaFileUploadAndDownload{})
...
func (e *FileUploadAndDownloadService) UploadFile(header *multipart.FileHeader, noSave string, classId int) (file example.ExaFileUploadAndDownload, err error) {
	...
	if noSave == "0" {
		...
		err = global.LRAG_DB.Where(&example.ExaFileUploadAndDownload{Key: key}).First(&existingFile).Error
