...
const showLoading = (
  ...
  const loadDom = document.getElementById('lrag-base-load-dom')
  activeAxios++
    ...
    if (activeAxios > 0 && !isLoadingVisible) {
      ...
      forceCloseTimer = setTimeout(() => {
        if (isLoadingVisible && loadingInstance) {
          console.warn(i18n.global.t('common.request.loadForceCloseWarn'))
...
const resetLoading = () => {
  ...
  if (loadingInstance) {
    try {
      ...
      console.warn(i18n.global.t('common.request.loadCloseWarn'), e)
    }
    loadingInstance = null
  }
}
...
service.interceptors.request.use(
  ...
  (error) => {
    ...
    emitter.emit('show-error', {
      code: 'request',
      message: error.message || i18n.global.t('common.request.sendFailed')
    })
    return error
  }
)
...
function getErrorMessage(error) {
  ...
  if (status === 429) {
    return i18n.global.t('common.request.tooManyRequests')
  }
  if (status === 504 && !serverMsg) {
    return i18n.global.t('common.request.gatewayTimeout')
  }
  return (
    serverMsg ||
    error.response?.statusText ||
    i18n.global.t('common.request.failed')
  )
}
