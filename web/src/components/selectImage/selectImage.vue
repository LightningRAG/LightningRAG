<template>
  <div>
    ...
    <el-drawer v-model="drawer" :title="$t('comp.mediaLib.drawerTitle')" :size="880">
      <div class="flex">
            ...
            >
              <template #default="{ node, data }">
                ...
                <el-dropdown>
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item @click="addCategoryFun(data)">{{ $t('comp.mediaLib.addCategory') }}</el-dropdown-item>
                      <el-dropdown-item @click="editCategory(data)" v-if="data.ID > 0">{{ $t('comp.mediaLib.editCategory') }}</el-dropdown-item>
                      <el-dropdown-item @click="deleteCategoryFun(data.ID)" v-if="data.ID > 0">{{ $t('comp.mediaLib.deleteCategory') }}</el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
              </template>
        ...
        <div class="ml-4 w-[605px]">
          <div class="lrag-btn-list gap-2">
            <el-input v-model.trim="search.keyword" class="w-96" :placeholder="$t('comp.mediaLib.phSearchFile')" clearable />
            <el-button type="primary" icon="search" @click="onSubmit"></el-button>
          </div>
          <div class="lrag-btn-list gap-2">
            <el-button @click="useSelectedImages" type="danger" :disabled="selectedImages.length === 0" :icon="ArrowLeftBold">{{ $t('comp.mediaLib.btnSelect') }}</el-button>
                ...
                <el-image :key="key" :src="getUrl(item.url)" fit="cover" class="w-full h-full relative" @click="toggleImageSelection(item)" :class="{ selected: isSelected(item) }">
                  <template #error>
                    ...
                    >
                      ...
                      {{ $t('comp.mediaLib.videoNotSupported') }}
                    </video>
    ...
    <el-dialog v-model="categoryDialogVisible" @close="closeAddCategoryDialog" width="520"
               :title="categoryFormData.ID === 0 ? $t('comp.mediaLib.addCategory') : $t('comp.mediaLib.editCategory')"
               draggable
    >
      <el-form ref="categoryForm" :rules="rules" :model="categoryFormData" label-width="80px">
        <el-form-item :label="$t('comp.mediaLib.parentCategory')">
          <el-tree-select
        ...
        <el-form-item :label="$t('comp.mediaLib.categoryName')" prop="name">
          <el-input v-model.trim="categoryFormData.name" :placeholder="$t('comp.mediaLib.categoryName')"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
...
const editFileNameFunc = async(row) => {
  ElMessageBox.prompt(t('comp.mediaLib.phEditFileName'), t('common.edit'), {
    confirmButtonText: t('common.ok'),
    cancelButtonText: t('common.cancel'),
    inputPattern: /\S/,
    inputErrorMessage: t('comp.mediaLib.ruleNotEmpty'),
    inputValue: row.name
  }).then(async({ value }) => {
    row.name = value
    ...
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: t('comp.mediaLib.editOk')
      })
      await getImageList()
    }
  }).catch(() => {
    ElMessage({
      type: 'info',
      message: t('comp.mediaLib.editCancelled')
    })
  })
}
...
const chooseImg = (url) => {
  if (props.fileType) {
    ...
    if (!typeSuccess) {
      ElMessage({
        type: 'error',
        message: t('comp.mediaLib.unsupportedType')
      })
      return
    }
  }
...
const deleteCheck = (item) => {
  ElMessageBox.confirm(t('comp.mediaLib.confirmDeleteFile'), t('common.tipTitle'), {
    confirmButtonText: t('common.ok'),
    cancelButtonText: t('common.cancel'),
    type: 'warning'
  }).then(async() => {
    ...
    ElMessage({
      type: 'info',
      message: t('comp.mediaLib.deleteCancelled')
    })
  })
}
...
const fetchCategories = async() => {
  ...
  let data = {
    name: t('comp.mediaLib.allCategories'),
    ID: 0,
    pid: 0,
    children:[]
  }
...
const rules = computed(() => ({
  name: [
    { required: true, message: t('comp.mediaLib.ruleCategoryName'), trigger: 'blur' },
    { max: 20, message: t('comp.mediaLib.ruleCategoryMax20'), trigger: 'blur' }
  ]
}))
...
const deleteCategoryFun = async(id) => {
  ...
  if (res.code === 0) {
    ElMessage.success({ type: 'success', message: t('common.deleteOk') })
    await fetchCategories()
  }
}
...
const confirmAddCategory = async() => {
  categoryForm.value.validate(async valid => {
    if (valid) {
      ...
      if (res.code === 0) {
        ElMessage({ type: 'success', message: t('comp.mediaLib.opOk') })
        await fetchCategories()
