<template>
  <div>
    <el-upload
      ...
      class="upload-btn"
    >
      <el-button type="primary" :icon="Upload">{{ $t('common.upload.btnNormal') }}</el-button>
    </el-upload>
  </div>
</template>
...
<script setup>
  ...
  const checkFile = (file) => {
    ...
    let pass = true
    if (!isVideo && !isImage) {
      ElMessage.error(t('common.upload.wrongMime'))
      fullscreenLoading.value = false
      pass = false
    }
    if (!isLt5M && isVideo) {
      ElMessage.error(t('common.upload.videoMax5mb'))
      fullscreenLoading.value = false
      pass = false
    }
    if (!isLt500K && isImage) {
      ElMessage.error(t('common.upload.imageMax500k'))
      fullscreenLoading.value = false
      pass = false
    }
  ...
  const uploadError = () => {
    ElMessage({
      type: 'error',
      message: t('common.upload.failed')
    })
    fullscreenLoading.value = false
  }
</script>
