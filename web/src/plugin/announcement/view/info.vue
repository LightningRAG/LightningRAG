<template>
  <div>
    <div class="lrag-search-box">
      ...
      >
        <el-form-item :label="t('example.announcement.createdAtLabel')" prop="createdAt">
          <template #label>
            <span>
              {{ t('example.announcement.createdAtLabel') }}
              <el-tooltip :content="t('example.announcement.dateRangeHint')">
          ...
          <el-date-picker
            ...
            type="datetime"
            :placeholder="t('example.announcement.startDate')"
            :disabled-date="
              (time) =>
          ...
          <el-date-picker
            ...
            type="datetime"
            :placeholder="t('example.announcement.endDate')"
            :disabled-date="
              (time) =>
        ...
        <el-form-item>
          ...
          >
            {{ t('example.announcement.collapse') }}
          </el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="lrag-table-box">
      ...
      >
        ...
        <el-table-column align="left" :label="$t('common.colTitle')" prop="title" width="120" />
        <el-table-column align="left" :label="$t('common.colAuthor')" prop="userID" width="120">
          <template #default="scope">
            <span>{{
        ...
        <el-table-column :label="$t('common.colAttachments')" prop="attachments" width="200">
          <template #default="scope">
            <div class="file-list">
              <el-tag
        ...
        >
          <template #default="scope">
            ...
            >
              {{ t('example.announcement.btnChange') }}
            </el-button>
            <el-button
              type="primary"
              link
    ...
    >
      <template #header>
        <div class="flex justify-between items-center">
          <span class="text-lg">{{
            type === 'create' ? t('example.announcement.drawerAdd') : t('example.announcement.drawerEdit')
          }}</span>
          <div>
            <el-button type="primary" @click="enterDialog">{{ t('example.announcement.confirmOk') }}</el-button>
            <el-button @click="closeDialog">{{ t('example.announcement.confirmCancel') }}</el-button>
          </div>
        </div>
      </template>
...
<script setup>
    ...
    createdAt: [
      {
        validator: (rule, value, callback) => {
          if (
            ...
            callback(new Error(i18n.global.t('common.dateNeedEnd')))
          } else if (
            !searchInfo.value.startCreatedAt &&
            searchInfo.value.endCreatedAt
          ) {
            callback(new Error(i18n.global.t('common.dateNeedStart')))
          } else if (
            searchInfo.value.startCreatedAt &&
            searchInfo.value.endCreatedAt &&
            ...
            callback(new Error(i18n.global.t('common.dateStartBeforeEnd')))
          } else {
            callback()
          }
        },
  ...
  // 删除行
  const deleteRow = (row) => {
    ElMessageBox.confirm(t('example.announcement.confirmDelete'), t('common.tipTitle'), {
  ...
  const onDelete = async () => {
    ElMessageBox.confirm(t('example.announcement.confirmDelete'), t('common.tipTitle'), {
      ...
      if (multipleSelection.value.length === 0) {
        ElMessage({
          type: 'warning',
          message: t('example.announcement.pickToDelete')
        })
        return
      }
      ...
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: t('example.announcement.deleteOk')
        })
