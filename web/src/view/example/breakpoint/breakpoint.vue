<template>
  <div class="break-point">
    <div class="lrag-table-box">
      <el-divider content-position="left">{{ t('example.breakpoint.dividerLargeFile') }}</el-divider>
      <form id="fromCont" method="post">
        <!-- 新增按钮容器，使用 Flexbox 对齐按钮 -->
        <div class="button-container">
          <div class="fileUpload" @click="inputChange">
            <span class="takeFile">{{ t('example.breakpoint.chooseFile') }}</span>
            <input
              v-show="false"
              id="file"
              ref="FileInput"
          ...
          <el-button
            ...
            class="uploadBtn"
            @click="getFile"
          >{{ t('example.breakpoint.uploadFile') }}</el-button>
        </div>
      </form>
      <div class="el-upload__tip">{{ t('example.breakpoint.tipMaxSize') }}</div>
      <div class="list">
  ...
  if (file.value.size < maxSize) {
    ...
    fileR.onload = async (e) => {
      ...
      if (!IsFinish) {
        ...
        ElMessage.success(t('example.breakpoint.instantOk'))
      }
      waitNum.value = waitUpLoad.value.length // 记录长度用于百分比展示
    }
  } else {
    limitFileSize.value = true
    ElMessage(t('example.breakpoint.fileTooBig'))
  }
}
...
const getFile = () => {
  // 确定按钮
  if (file.value === null) {
    ElMessage(t('example.breakpoint.selectFileFirst'))
    return
  }
  // 检查文件上传进度
  if (percentage.value === 100) {
    ElMessage.success(t('example.breakpoint.uploadDone'))
...
const upLoadFileSlice = async (item) => {
  ...
  if (waitNum.value === 0) {
    ...
    if (res.code === 0) {
      ...
      ElMessage.success(t('example.breakpoint.uploadSuccess'))
