<template>
  <div class="grid grid-cols-2 gap-6 lrag-theme-font px-6">
    <div
      v-for="layout in layoutModes"
      :key="layout.value"
      class="lrag-theme-layout-card"
      :class="{
    ...
    >
      ...
      <div class="text-center">
        <span class="block text-base font-semibold lrag-theme-text-main mb-2" :class="{ 'text-current': modelValue === layout.value }" :style="modelValue === layout.value ? { color: primaryColor } : {}">{{ layout.label }}</span>
...
const layoutModes = computed(() =>
  layoutModeDefs.map((def) => ({
    ...def,
    label: t(`layoutModes.${def.value}.label`),
    description: t(`layoutModes.${def.value}.description`)
  }))
)
