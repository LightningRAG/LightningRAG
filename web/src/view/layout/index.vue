<template>
  ...
  >
    ...
    />
    <lrag-header />
    <div class="flex flex-row w-full lrag-container pt-16 box-border !h-full">
      <lrag-aside
        v-if="
      ...
      <div class="flex-1 w-0 h-full">
        <lrag-tabs v-if="config.showTabs" />
        <div
          class="overflow-auto px-2"
          :class="config.showTabs ? 'lrag-container2' : 'lrag-container pt-1'"
        >
          <router-view v-if="reloadFlag" v-slot="{ Component, route }">
            <div
              id="lrag-base-load-dom"
              class="lrag-body-h bg-gray-50 dark:bg-slate-800"
            >
              <transition
                mode="out-in"
...
<script setup>
  import LragAside from '@/view/layout/aside/index.vue'
  import LragHeader from '@/view/layout/header/index.vue'
  import useResponsive from '@/hooks/responsive'
  import LragTabs from './tabs/index.vue'
