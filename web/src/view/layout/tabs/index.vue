<template>
  <div class="lrag-tabs">
    ...
    <ul
      ...
      class="contextmenu"
    >
      <li @click="closeAll">{{ $t('layout.tabs.closeAll') }}</li>
      <li @click="closeLeft">{{ $t('layout.tabs.closeLeft') }}</li>
      <li @click="closeRight">{{ $t('layout.tabs.closeRight') }}</li>
      <li @click="closeOther">{{ $t('layout.tabs.closeOther') }}</li>
    </ul>
  </div>
</template>
...
<script setup>
  ...
  const tabItemTitle = (item) => {
    void locale.value
    return resolveMenuTitle(item.meta, item)
  }
