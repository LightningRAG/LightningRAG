<template>
  <div>
      ...
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline"
        ...
        <el-form-item :label="$t('tools.exportTemplate.labelCreatedDate')" prop="createdAtRange">
          <template #label>
            <span>
              ...
              <el-tooltip :content="$t('tools.exportTemplate.dateRangeTip')">
                <el-icon>
                  <QuestionFilled />
                </el-icon>
              </el-tooltip>
          ...
          <el-date-picker v-model="searchInfo.createdAtRange" class="w-[380px]" type="datetimerange"
            :range-separator="$t('sysError.rangeTo')"
            :start-placeholder="$t('sysError.startTime')" :end-placeholder="$t('sysError.endTime')" />
        </el-form-item>

        <el-form-item :label="$t('tools.release.colVersionName')" prop="versionName">
          <el-input v-model="searchInfo.versionName" :placeholder="$t('common.phSearchCondition')" />
        </el-form-item>

        <el-form-item :label="$t('tools.release.colVersionCode')" prop="versionCode">
          <el-input v-model="searchInfo.versionCode" :placeholder="$t('common.phSearchCondition')" />
        </el-form-item>
        ...
        <el-form-item>
          ...
          <el-button link type="primary" icon="arrow-down" @click="showAllQuery = true"
            v-if="!showAllQuery">{{ $t('sysError.expand') }}</el-button>
          <el-button link type="primary" icon="arrow-up" @click="showAllQuery = false" v-else>{{ $t('sysError.collapse') }}</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="lrag-table-box">
      <div class="lrag-btn-list">
        <el-button type="success" icon="download" @click="openExportDialog">{{ $t('tools.release.btnCreateRelease') }}</el-button>
        <el-button type="warning" icon="upload" @click="openImportDialog">{{ $t('tools.release.btnImportVersion') }}</el-button>
        <el-button icon="delete" style="margin-left: 10px;" :disabled="!multipleSelection.length"
          @click="onDelete">{{ $t('common.delete') }}</el-button>
      </div>
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" />

        <el-table-column sortable align="left" :label="$t('common.colDate')" prop="CreatedAt" width="180">
          <template #default="scope">{{ formatDate(scope.row.CreatedAt) }}</template>
        </el-table-column>

        <el-table-column align="left" :label="$t('tools.release.colVersionName')" prop="versionName" width="120" />

        <el-table-column align="left" :label="$t('tools.release.colVersionCode')" prop="versionCode" width="120" />

        <el-table-column align="left" :label="$t('common.colActions')" fixed="right" min-width="320">
          <template #default="scope">
            <el-button type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon
                style="margin-right: 5px">
                <InfoFilled />
              </el-icon>{{ $t('common.view') }}</el-button>
            <el-button type="success" link icon="download" class="table-button"
              @click="downloadJson(scope.row)">{{ $t('tools.release.btnDownloadPackage') }}</el-button>
            <el-button type="primary" link icon="delete" @click="deleteRow(scope.row)">{{ $t('common.delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="detailShow" :show-close="true"
      :before-close="closeDetailShow" :title="$t('tools.release.drawerViewTitle')">
      <el-descriptions :column="1" border>
        <el-descriptions-item :label="$t('tools.release.colVersionName')">
          {{ detailForm.versionName }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('tools.release.colVersionCode')">
          {{ detailForm.versionCode }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('tools.release.colDescription')">
    ...
    <!-- 导出版本抽屉 -->
    <el-drawer v-model="exportDialogVisible" :title="$t('tools.release.btnCreateRelease')" direction="rtl" size="80%" :before-close="closeExportDialog"
      :show-close="false">
      <template #header>
        <div class="flex justify-between items-center">
          <span class="text-lg">{{ $t('tools.release.btnCreateRelease') }}</span>
          <div>
            <el-button @click="closeExportDialog">{{ $t('common.cancel') }}</el-button>
            <el-button type="primary" @click="handleExport" :loading="exportLoading">{{ $t('tools.release.btnCreateRelease') }}</el-button>
          </div>
        </div>
      </template>
      <el-form :model="exportForm" label-width="100px">
        <el-form-item :label="$t('tools.release.colVersionName')" required>
          <el-input v-model="exportForm.versionName" :placeholder="$t('tools.release.phVersionName')" />
        </el-form-item>
        <el-form-item :label="$t('tools.release.colVersionCode')" required>
          <el-input v-model="exportForm.versionCode" :placeholder="$t('tools.release.phVersionCode')" />
        </el-form-item>
        <el-form-item :label="$t('tools.release.colDescription')">
          <el-input v-model="exportForm.description" type="textarea" :placeholder="$t('tools.release.phVersionDesc')" />
        </el-form-item>
        <el-form-item :label="$t('tools.release.releaseBundle')">
          <div class="flex gap-3 w-full">
            <!-- 菜单选择 -->
            <div class="card-col card-vertical">
              <div class="card-header">
                <span class="card-title">{{ $t('tools.release.pickMenus') }}</span>
              </div>
              <div class="card-filter">
                <el-input v-model="menuFilterText" :placeholder="$t('tools.release.filterKeyword')" clearable size="small" />
              </div>
              <div class="card-body">
            ...
            <div class="card-col card-vertical">
              <div class="card-header">
                <span class="card-title">{{ $t('tools.release.pickApis') }}</span>
              </div>
              <div class="card-filter">
                <el-input v-model="apiFilterTextName" :placeholder="$t('tools.release.filterByName')" clearable size="small"
                  style="margin-bottom: 8px" />
                <el-input v-model="apiFilterTextPath" :placeholder="$t('tools.release.filterByPath')" clearable size="small" />
              </div>
              <div class="card-body">
            ...
            <div class="card-col card-vertical">
              <div class="card-header">
                <span class="card-title">{{ $t('tools.release.pickDicts') }}</span>
              </div>
              <div class="card-filter">
                <el-input v-model="dictFilterText" :placeholder="$t('tools.release.filterKeyword')" clearable size="small" />
              </div>
              <div class="card-body">
                <el-tree ref="dictTreeRef" :data="dictTreeData" :default-checked-keys="selectedDictIds"
                  ...
                  <template #default="{ data }">
                    <div class="flex items-center justify-between w-full pr-1">
                      ...
                      <el-tooltip :content="data.desc || (data.value ? $t('tools.release.valueColon', { v: data.value }) : '')">
                        <span class="text-gray-500 dark:text-gray-400 text-xs ml-2">
                          {{ data.type || (data.value ? $t('tools.release.valueColon', { v: data.value }) : '') }}
                        </span>
                      </el-tooltip>
                    </div>
                  </template>
    ...
    <!-- 导入版本抽屉 -->
    <el-drawer v-model="importDialogVisible" :title="$t('tools.release.btnImportVersion')" direction="rtl" size="80%" :before-close="closeImportDialog"
      :show-close="false">
      <template #header>
        <div class="flex justify-between items-center">
          <span class="text-lg">{{ $t('tools.release.btnImportVersion') }}</span>
          <div>
            ...
            <el-button type="primary" @click="handleImport" :loading="importLoading"
              :disabled="!importJsonContent.trim()">{{ $t('tools.release.btnImport') }}</el-button>
          </div>
        </div>
      </template>
      <el-form label-width="100px">
        <el-form-item :label="$t('tools.release.labelUploadFile')">
          <el-upload ref="uploadRef" :auto-upload="false" :show-file-list="true" :limit="1" accept=".json"
            ...
            <div class="el-upload__text">
              {{ $t('tools.release.uploadJsonLine1') }}<em>{{ $t('tools.release.clickUpload') }}</em>
            </div>
            <template #tip>
              <div class="el-upload__tip">
                {{ $t('tools.release.jsonOnly') }}
              </div>
            </template>
          </el-upload>
        </el-form-item>
        <el-form-item :label="$t('tools.release.labelVersionJson')">
          <el-input v-model="importJsonContent" type="textarea" :rows="20" :placeholder="$t('tools.release.phPasteVersionJson')"
            @input="handleJsonContentChange" />
        </el-form-item>
        <el-form-item :label="$t('tools.release.previewContent')" v-if="importPreviewData">
          <div class="preview-wrap">
            <div class="flex gap-3 w-full">
              <div class="card-col">
                <div class="card-vertical">
                  <div class="card-header">
                    <h3 class="card-title">{{ $t('tools.release.menuCountTitle', { n: getTotalMenuCount() }) }}</h3>
                  </div>
                  <div class="card-body">
              ...
              <div class="card-col">
                <div class="card-vertical">
                  <div class="card-header">
                    <h3 class="card-title">{{ $t('tools.release.apiCountTitle', { n: importPreviewData.apis?.length || 0 }) }}</h3>
                  </div>
                  <div class="card-body">
              ...
              <div class="card-col">
                <div class="card-vertical">
                  <div class="card-header">
                    <h3 class="card-title">{{ $t('tools.release.dictCountTitle', { n: importPreviewData.dictionaries?.length || 0 }) }}</h3>
                  </div>
                  <div class="card-body">
...
import { UploadFilled, QuestionFilled, InfoFilled } from '@element-plus/icons-vue'
...
const dictTreeProps = computed(() => ({
  ...
  label: function (data) {
    ...
    return t('tools.release.unknownDictItem')
  }
}))
...
// 构建API树形结构
const buildApiTree = (apis, translate) => {
  const apiObj = {}
  apis.forEach((item) => {
...
const handleExport = async () => {
  if (!exportForm.value.versionName || !exportForm.value.versionCode) {
    ElMessage.warning(t('tools.release.fillNameAndCode'))
    return
  }
  ...
  try {
    ...
    if (res.code !== 0) {
      ElMessage.error(res.msg || t('tools.release.exportFail'))
      return
    }

    ElMessage.success(t('tools.release.exportOk'))
    closeExportDialog()
    getTableData() // 刷新表格数据
  } catch (error) {
...
const handleFileChange = (file) => {
  ...
  if (!file.name.toLowerCase().endsWith('.json')) {
    ElMessage.error(t('tools.release.jsonUploadInvalid'))
    uploadRef.value.clearFiles()
    return
  }
  ...
  reader.onload = (e) => {
    try {
      ...
      ElMessage.success(t('tools.release.fileUploadOk'))
    } catch (error) {
      ElMessage.error(t('tools.release.jsonFileInvalid'))
      uploadRef.value.clearFiles()
    }
  }
...
const handleImport = async () => {
  if (!importJsonContent.value.trim()) {
    ElMessage.warning(t('tools.release.enterVersionJson'))
    return
  }
  ...
  try {
    ...
    ElMessage.error(t('tools.release.jsonInvalid'))
    return
  }
  ...
  try {
    ...
    if (res.code === 0) {
      ElMessage.success(t('tools.release.importOk'))
      closeImportDialog()
      getTableData() // 刷新表格数据
    } else {
