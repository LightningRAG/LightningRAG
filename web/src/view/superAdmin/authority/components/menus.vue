  ...
  <div>
    <div class="sticky top-0.5 z-10 pb-2">
      <div class="flex gap-2 items-center mb-2">
        <el-input v-model="filterText" class="flex-1" :placeholder="$t('admin.roleMenus.filterPh')" />
        <el-button type="primary" @click="relation">{{ $t('common.ok') }}</el-button>
      </div>
      <div class="flex items-center gap-2">
        <span class="whitespace-nowrap">{{ $t('admin.roleMenus.defaultHomeLabel') }}</span>
        <el-select
          :model-value="row.defaultRouter"
          filterable
          :placeholder="$t('admin.roleMenus.phDefaultHome')"
          class="flex-1"
            ...
            <div class="flex items-center gap-2">
              ...
              <span v-if="data.menuBtn.length">
                <el-button type="primary" link @click.stop="() => OpenBtn(data)">
                  {{ $t('admin.roleMenus.assignButtons') }}
                </el-button>
              </span>
            </div>
          </template>
    ...
    <el-dialog v-model="btnVisible" :title="$t('admin.roleMenus.dialogAssignButtons')" destroy-on-close>
      <el-table
        ref="btnTableRef"
        :data="btnData"
        row-key="ID"
      ...
      >
        ...
        <el-table-column :label="$t('admin.roleMenus.colBtnName')" prop="name" />
        <el-table-column :label="$t('admin.roleMenus.colBtnRemark')" prop="desc" />
      </el-table>
      <template #footer>
...
<script setup>
  ...
  const menuDefaultProps = ref({
    children: 'children',
    label: function (data) {
      return resolveMenuTitle(data.meta, route)
    },
  ...
  const buildOptionsFromCheckedLeafMenus = () => {
    ...
    const options = checkedLeafMenus
      ...
      .map((item) => ({
        label: resolveMenuTitle(item?.meta, route) || item.name,
        value: item.name
      }))
    ...
    if (props.row.defaultRouter && !options.some(o => o.value === props.row.defaultRouter)) {
      ...
      if (found && !isExternalRoute(found.name)) {
        options.push({
          label: resolveMenuTitle(found?.meta, route) || found.name,
          value: found.name
        })
      }
    }
  ...
  const handleDefaultRouterChange = (val) => {
    ...
    if (!isDefaultRouterAllowed(val)) {
      ElMessage.warning(t('admin.roleMenus.warnHomeNeedChecked'))
      return
    }
    setDefault({ name: val })
  }
  ...
  const relation = async () => {
    ...
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: t('admin.roleMenus.menuSaveOk')
      })
  ...
  const enterDialog = async () => {
    ...
    if (res.code === 0) {
      ElMessage({ type: 'success', message: t('admin.roleMenus.btnSaveOk') })
      btnVisible.value = false
    }
  }
  ...
  const filterNode = (value, data) => {
    ...
    const label = resolveMenuTitle(data.meta, route)
    const raw = String(data.meta?.title ?? '')
    return (
